...
function swap(a, i, j) {
  ...
  // A DNA object is an order through the cities.cities
  ...
  DNA.prototype.calcDistance = function() {
    ...
    for (var i = 0; i < this.order.length - 1; i++) {
      ...
      var cityA = cities.cities[cityAIndex];
      var cityBIndex = this.order[i + 1];
      var cityB = cities.cities[cityBIndex];
  ...
  DNA.prototype.show = function() {
    ...
    for (var i = 0; i < this.order.length; i++) {
      var n = this.order[i];
      vertex(cities.cities[n].x, cities.cities[n].y);
    }
    endShape();
    ...
    for (var i = 0; i < this.order.length; i++) {
      var n = this.order[i];
      ellipse(cities.cities[n].x, cities.cities[n].y, 8, 8);
    }
  }
